  ...
  return (
    <>
      {isDragging && (
        <div
          style={style}
          className="fixed top-0 left-0 z-50 bg-white dark:bg-neutral-800 rounded-lg shadow-2xl opacity-90 pointer-events-none"
        />
      )}
      {showDragHandle && (
        <div className="absolute left-2 top-1/2 -translate-y-1/2 cursor-grab active:cursor-grabbing opacity-0 group-hover:opacity-100 transition-opacity">
          <GripHorizontal className="text-slate-400 dark:text-slate-600" size={20} />
        </div>
      )}
    </>
  )
