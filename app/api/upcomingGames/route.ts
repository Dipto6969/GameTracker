...
export async function GET() {
  try {
    ...
    // Using a future date for upcoming games
    const futureDate = new Date()
    futureDate.setDate(futureDate.getDate() + 365) // Next 365 days
    const futureDateStr = futureDate.toISOString().split('T')[0]

    const response = await fetch(
      `https://api.rawg.io/api/games?key=${process.env.RAWG_API_KEY}&dates=${new Date().toISOString().split('T')[0]},${futureDateStr}&ordering=-released&page_size=50`,
